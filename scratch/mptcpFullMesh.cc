...
int main(int argc, char *argv[])
{
  ...
  // Create a full mesh topology by connecting each node to every other node
  for (uint32_t i = 0; i < nodes.GetN(); ++i)
  {
    for (uint32_t j = i + 1; j < nodes.GetN(); ++j)
    {
      PointToPointHelper meshLink;
      meshLink.SetDeviceAttribute("DataRate", StringValue("100Mbps"));
      meshLink.SetChannelAttribute("Delay", StringValue("1ms"));

      NetDeviceContainer meshDevices;
      meshDevices = meshLink.Install(nodes.Get(i), nodes.Get(j));
    }
  }
